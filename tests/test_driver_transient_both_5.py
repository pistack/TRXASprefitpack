...
def test_driver_transient_both_5():
    tau_1 = 1
    tau_2 = 10
    tau_3 = 1000
    fwhm = 0.08
    ...
    y_obs_1 = rate_eq_conv(t_seq-t0[0], fwhm, abs_1, eigval_seq, V_seq, c_seq, irf='c') + \
        dmp_osc_conv(t_seq-t0[0], fwhm, tau_osc, period_osc, phase_1, abs_osc_1, 'c')
    y_obs_2 = rate_eq_conv(t_seq-t0[0], fwhm, abs_2, eigval_seq, V_seq, c_seq, irf='c') + \
        dmp_osc_conv(t_seq-t0[0], fwhm, tau_osc, period_osc, phase_2, abs_osc_2, 'c')
    y_obs_3 = rate_eq_conv(t_seq-t0[0], fwhm, abs_3, eigval_seq, V_seq, c_seq, irf='c') + \
        dmp_osc_conv(t_seq-t0[0], fwhm, tau_osc, period_osc, phase_3, abs_osc_3, 'c')
    y_obs_4 = rate_eq_conv(t_seq-t0[0], fwhm, abs_4, eigval_seq, V_seq, c_seq, irf='c') + \
        dmp_osc_conv(t_seq-t0[0], fwhm, tau_osc, period_osc, phase_4, abs_osc_4, 'c')
    ...
    result_ampgo_exp = fit_transient_exp('c', fwhm_init, t0_init, tau_init, True,
