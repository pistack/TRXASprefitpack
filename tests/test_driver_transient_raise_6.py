...
def test_driver_transient_raise_6():
    tau_1 = 0.2
    tau_2 = 2
    tau_3 = 10
    fwhm_G = 0.100
    ...
    save_TransientResult(result_ampgo, 'test_driver_transient_raise_6')
    load_result_ampgo = load_TransientResult('test_driver_transient_raise_6')
    os.remove('test_driver_transient_raise_6.h5')

    assert np.allclose(result_ampgo['x'], ans)
    assert str(result_ampgo) == str(load_result_ampgo)

if __name__ == '__main__':
    test_driver_transient_raise_6()
