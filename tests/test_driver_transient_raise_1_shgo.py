...
def test_driver_transient_raise_1_shgo():
    tau_1 = 0.2
    tau_2 = 2
    tau_3 = 10
    fwhm = 0.100
    ...
    result_ampgo = fit_transient_raise('g', fwhm_init, t0_init, tau_init, True,
    method_glb='shgo', bound_fwhm=bound_fwhm, bound_t0=bound_t0, bound_tau=bound_tau,
    t=t, intensity=intensity, eps=eps)

    save_TransientResult(result_ampgo, 'test_driver_transient_raise_1')
    load_result_ampgo = load_TransientResult('test_driver_transient_raise_1')
    os.remove('test_driver_transient_raise_1.h5')
