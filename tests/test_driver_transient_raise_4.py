...
def test_driver_transient_raise_4():
    tau_1 = 0.2
    tau_2 = 2
    tau_3 = 10
    fwhm = 0.100
    ...
    same_t0=True, t=t, intensity=intensity, eps=eps)

    save_TransientResult(result_ampgo, 'test_driver_transient_raise_4')
    load_result_ampgo = load_TransientResult('test_driver_transient_raise_4')
    os.remove('test_driver_transient_raise_4.h5')

    assert np.allclose(result_ampgo['x'], ans)
    assert str(result_ampgo) == str(load_result_ampgo)

if __name__ == '__main__':
    test_driver_transient_raise_4()
