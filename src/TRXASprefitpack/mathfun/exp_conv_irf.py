...
# hessian of sum of exponential decay convolved with gaussian irf

def hess_exp_sum_conv_gau(t: np.ndarray, fwhm: float,
                           k: np.ndarray, c: np.ndarray, base: Optional[bool] = True) -> np.ndarray:
    '''
    Compute hessian of sum of exponential function convolved with normalized gaussian
    distribution
    ...
    Returns:
     Hessian of Convolution of normalized gaussian distribution and sum of exponential
     decay :math:`(\\exp(-kt))`. (Ht, Hfwhm, Hk (diag))
    Note:

     Ht
     * 1st column: d^2f/dt^2
     * 2nd column: d^2f/dtd(fwhm)
     * 2+i th column: d^2f/dtdk_i

     Hfwhm
     * 1st column: d^2f/ d(fwhm)^2
     * 1+i th column: d^2f/d(fwhm)dk_i

     Hk (diag only)
     * ith column: d^2 f/dk_i^2
    '''
    hess_t = np.zeros((t.size, 2+k.size))
    hess_fwhm = np.zeros((t.size, 1+k.size))
    hess_k = np.zeros((t.size, k.size))
    for i in range(k.size):
        hess_i = hess_exp_conv_gau(t, fwhm, k[i])
        hess_t[:, 0] = hess_t[:, 0] + c[i]*hess_i[:, 0]
        hess_t[:, 1] = hess_t[:, 1] + c[i]*hess_i[:, 1]
        hess_t[:, 2+i] = c[i]*hess_i[:, 2]
        hess_fwhm[:, 0] = hess_fwhm[:, 0] + c[i]*hess_i[:, 3]
        hess_fwhm[:, 1+i] = c[i]*hess_i[:, 4]
        hess_k[:, i] = c[i]*hess_i[:, 5]  
    if base:
        hess_i = hess_exp_conv_gau(t, fwhm, 0)
        hess_t[:, 0] = hess_t[:, 0] + c[-1]*hess_i[:, 0]
        hess_t[:, 1] = hess_t[:, 1] + c[-1]*hess_i[:, 1]
        hess_fwhm[:, 0] = hess_fwhm[:, 0] + c[-1]*hess_i[:, 3]

    return hess_t, hess_fwhm, hess_k

def hess_exp_sum_conv_cauchy(t: np.ndarray, fwhm: float,
                           k: np.ndarray, c: np.ndarray, base: Optional[bool] = True) -> np.ndarray:
    '''
    Compute hessian of sum of exponential function convolved with normalized cauchy
    distribution
    ...
    Returns:
     Hessian of Convolution of normalized cauchy distribution and sum of exponential
     decay :math:`(\\exp(-kt))`. (Ht, Hfwhm, Hk (diag))
    Note:
    ...
    hess_k = np.zeros((t.size, k.size))
    for i in range(k.size):
        hess_i = hess_exp_conv_cauchy(t, fwhm, k[i])
        hess_t[:, 0] = hess_t[:, 0] + c[i]*hess_i[:, 0]
        hess_t[:, 1] = hess_t[:, 1] + c[i]*hess_i[:, 1]
        hess_t[:, 2+i] = c[i]*hess_i[:, 2]
        hess_fwhm[:, 0] = hess_fwhm[:, 0] + c[i]*hess_i[:, 3]
        ...
        hess_k[:, i] = c[i]*hess_i[:, 5]  
    if base:
        hess_i = hess_exp_conv_cauchy(t, fwhm, 0)
        hess_t[:, 0] = hess_t[:, 0] + c[-1]*hess_i[:, 0]
        hess_t[:, 1] = hess_t[:, 1] + c[-1]*hess_i[:, 1]
        hess_fwhm[:, 0] = hess_fwhm[:, 0] + c[-1]*hess_i[:, 3]
