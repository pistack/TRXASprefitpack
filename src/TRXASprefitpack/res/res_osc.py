...
def residual_dmp_osc_same_t0(x0: np.ndarray, num_comp: int, irf: str,
    ...
    chi = np.empty(count)
    tau_osc = x0[num_irf+num_dataset:
    num_irf+num_dataset+num_comp]
    period_osc = x0[num_irf+num_dataset+num_comp:
    num_irf+num_dataset+2*num_comp]

    end = 0
    t0_idx = num_irf
    for ti, d, e in zip(t, intensity, eps):
        A = np.empty((2*num_comp, d.shape[0]))
        t0 = x0[t0_idx]
        if irf == 'g':
            A[:, :] = \
...
def res_grad_dmp_osc_same_t0(x0: np.ndarray, num_comp: int, irf: str,
    ...
    num_param = num_irf+num_dataset+2*num_comp
    ...
    for ti, d, e in zip(t, intensity, eps):
        step = d.shape[0]
        A = np.empty((2*num_comp, step))
        A_grad_osc = np.empty((2*num_comp, step, 4))
        grad_osc = np.empty((step, 2*num_comp+2))
        ...
        for j in range(d.shape[1]):
            ...
            grad_osc[:, :2] = \
                np.tensordot(c[:], A_grad_osc[:, :, :2], axes=1)
            for i in range(num_comp):
                grad_osc[:, 2+i] = \
                    c[i]*A_grad_osc[i, :, 2] + \
                        c[num_comp+i] * \
                            A_grad_osc[num_comp+i, :, 2]
                grad_osc[:, 2+num_comp+i] = \
                    c[i]*A_grad_osc[i, :, 3] + \
                        c[num_comp+i] * \
                            A_grad_osc[num_comp+i, :, 3]
            ...
            grad[t0_idx] = grad[t0_idx] -chi[end:end+step]@grad_osc[:, 0]
            df[end:end+step, num_irf:num_irf+num_comp] = \
                np.einsum('j,ij->ij', -1/tau_osc**2,
                          grad_osc[:, 2:2+num_comp])
            df[end:end+step, num_irf+num_comp:] = \
                grad_osc[:, 2+num_comp:2+2*num_comp]
