...
def residual_voigt(params: np.ndarray, num_voigt: int, edge: Optional[str] = None,
    ...
    Args:
     params: parameter used for fitting
              ...
              param[0: num_voigt]: peak position e0_i for each voigt component

              param[num_voigt: 2*num_voigt]: fwhm_G of each voigt component

              param[2*num_voigt:3*num_voigt]: fwhm_L of each voigt component
     ...
     edge ({'gaussian', 'lorenzian'}): type of edge shape function
    ...
    e0 = np.empty(num_voigt, dtype=float)
    fwhm_G = np.empty(num_voigt, dtype=float)
    fwhm_L = np.empty(num_voigt, dtype=float)

    for i in range(num_voigt):
        e0[i] = params[i]
        fwhm_G[i] = params[num_voigt+i]
        fwhm_L[i] = params[2*num_voigt+i]
    ...
    if edge is not None:
        if edge == 'gaussian':
            A[num_voigt+1, :] = edge_gaussian(e-params[-2], params[-1])
        elif edge == 'lorenzian':
            A[num_voigt+1,:] = edge_lorenzian(e-params[-2], params[-1])
    
    if base_order is not None:
        A[num_voigt+2, :] = np.ones(e.size)
        for i in range(base_order):
            A[num_voigt+3+i] = e*A[num_voigt+2+i]
...
def jac_res_voigt(params: np.ndarray, num_voigt: int, edge: Optional[str] = None,
    ...
    if edge is not None:
        if edge == 'gaussian':
            df_edge = c[num_voigt+1]*deriv_edge_gaussian(e-params[-2], params[-1])
        elif edge == 'lorenzian':
            df_edge = c[num_voigt+1]*deriv_edge_lorenzian(e-params[-2], params[-1]) 
